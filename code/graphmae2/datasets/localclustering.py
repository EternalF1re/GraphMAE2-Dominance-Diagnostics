...
import numpy as np
try:
    from localgraphclustering import GraphLocal, approximate_PageRank
except ImportError:  # Optional preprocessing dependency; not needed for --help.
    GraphLocal = None
    approximate_PageRank = None
...
def step1_local_clustering(data, name, idx_split, ego_size=128, num_iter=1000, log_steps=10000, num_workers=16, method='acl', save_dir=None):
    if GraphLocal is None or approximate_PageRank is None:
        raise RuntimeError(
            "LC preprocessing requires the optional 'localgraphclustering' package"
        )
    if save_dir is None:

...
try:
    import wandb
except ImportError:  # Optional upstream experiment logging.
    wandb = None
...
from config_paths import load_runtime_config, resolve_config_path
...
def build_args():
    ...
    parser.add_argument("--config_path", type=str, default=None,
                        help="explicit flat GraphMAE2 runtime YAML; paper execution/provenance records are not accepted")
...
def load_best_configs(args):
    ...
    config_path = resolve_config_path(dataset_name, getattr(args, "config_path", None))
    configs = load_runtime_config(config_path)
...
class WandbLogger(object):
    ...
    def start(self):
        if wandb is None:
            raise RuntimeError("wandb logging requested, but the optional 'wandb' package is not installed")
